"""Validates OSMIUM Bot 2 quote rule and reports quote-match metrics."""
...
FV_AND_BOOK = Path("calibration/round1/data/fv_and_book_ash_coated_osmium.json")
...
def _load_from_bots_json(product_dir: Path, bot_key: str = "bot2") -> dict:
...
def validate(
    ...
    bot2 = [t for t in tagged if t.bot == "bot2"]
    ...
    # presence rate
    raw = json.loads(fv_and_book.read_text())
    ...
    return {
        ...
        "presence_rate": n_bot2_ts / total_ts if total_ts > 0 else 0.0,
        "n_bot2_ts": n_bot2_ts,
        "n_total_ts": total_ts,
    }
...
def main() -> None:
    ...
    if args.bid_op is None:
      ...
      b = _load_from_bots_json(product_dir, "bot2")
