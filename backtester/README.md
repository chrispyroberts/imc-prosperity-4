# Prosperity Backtester Tooling

This directory now ships two CLIs:

- `prosperity3bt` for legacy historical replay compatibility
- `prosperity4mcbt` for the Rust-backed Prosperity 4 Monte Carlo workflow
...
Note: the bundled frontend in this repo is now Monte Carlo-only. `prosperity3bt --vis` is no longer supported by the shipped UI.
